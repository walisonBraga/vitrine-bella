...
export class LojaUsersManagementComponent implements OnInit, OnDestroy {
  ...
  loadUsers(): void {
    ...
    this.lojaUsersService.getLojaUsers()
      ...
      .subscribe({
        next: (users) => {
          // Filtrar apenas usuários da loja (excluir clientes)
          this.users = users.filter(user => {
            // Verificar tanto 'role' quanto 'userRole'
            const userRole = user.role || user.userRole;
            const hasValidRole = userRole &&
              Array.isArray(userRole) ?
              userRole.some(role => ['admin', 'store_owner', 'store_manager', 'store_employee'].includes(role)) :
              ['admin', 'store_owner', 'store_manager', 'store_employee'].includes(userRole);

            console.log(`Usuário ${user.fullName} (role: ${user.role}, userRole: ${user.userRole}) - Válido: ${hasValidRole}`);
            return hasValidRole;
          });
          this.loading = false;
        },
  ...
  async deleteUser(user: LojaUser): Promise<void> {
    // Verificar se é admin - não permitir exclusão
    const isAdmin = user.userRole === 'admin' || user.role === 'admin';
    if (isAdmin) {
      this.snackBar.open('Não é possível excluir contas de administrador por segurança', 'Fechar', { duration: 5000 });
      return;
    }
  ...
  getAdminCount(): number {
    return this.users.filter(u => (u.role || u.userRole) === 'admin').length;
  }

  getStoreOwnerCount(): number {
    return this.users.filter(u => (u.role || u.userRole) === 'store_owner').length;
  }

  getStoreEmployeeCount(): number {
    return this.users.filter(u => (u.role || u.userRole) === 'store_employee').length;
  }

  getStoreManagerCount(): number {
    return this.users.filter(u => (u.role || u.userRole) === 'store_manager').length;
  }
